...
<a href="https://github.com/CCI-Bonn/OHIF-AI">OHIF-AI</a> extends the OHIF DICOM Web viewer with browser-based interactive segmentation, either visual prompts (points, scribbles, lassos, bounding boxes) using models such as nnInteractive, SAM2, MedSAM2, and SAM3, or text prompts using VoxTell. It also supports AI-assisted radiology-style reports from medical images via multiple VLMs: local MedGemma (Hugging Face), cloud APIs (Gemini, GPT, Claude), the Hugging Face inference router (Kimi, Qwen, Gemma 4), or a self-hosted OpenAI-compatible server such as vLLM for open-weight multimodal models, so segmentation and reporting stay in one web workflow.
</li></ul>
